...
class EmployeeProfile(db.Model):
    ...
    phone = db.Column(db.String(100), nullable=True)
    department = db.Column(db.String(200), nullable=True)
    position_title = db.Column(db.String(200), nullable=True)
    employment_status = db.Column(db.String(100), nullable=True, default="Active")
    hire_date = db.Column(db.Date, nullable=True)
    ...
    disciplinary_records = db.relationship(
        "DisciplinaryRecord",
...
class PointLog(db.Model):
    ...
    employee = db.relationship("EmployeeProfile", back_populates="point_logs")
...
class DisciplinaryRecord(db.Model):
    ...
    employee = db.relationship("EmployeeProfile", back_populates="disciplinary_records")
...
class SOPRequirement(db.Model):
    ...
    sop_title = db.Column(db.String(200), nullable=False)
    linked_job_description = db.Column(db.String(200), nullable=True)
    status = db.Column(db.String(100), nullable=True, default="Pending")
    due_date = db.Column(db.Date, nullable=True)
    notes = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)

    employee = db.relationship("EmployeeProfile", back_populates="sop_requirements")
...
def create_app() -> Flask:
    ...
    def not_found(_error):
        return render_template("404.html"), 404 if template_exists(app, "404.html") else ("Page not found", 404)
    ...
    def internal_error(_error):
        ...
        return (
            ...
            500,
        ) if template_exists(app, "500.html") else ("Internal server error", 500)
    ...
    def dashboard():
        ...
        revenue_total = (
            db.session.query(func.coalesce(func.sum(Project.contract_value), 0.0)).scalar()
            or 0.0
        )
        ...
        monthly_invoice_rows = (
            db.session.query(
                ...
                func.coalesce(func.sum(Invoice.amount), 0.0).label("total"),
            )
            .filter(func.extract("year", Invoice.created_at) == datetime.utcnow().year)
            .group_by(func.extract("month", Invoice.created_at))
            .order_by(func.extract("month", Invoice.created_at))
            .all()
        )

        month_names = ["Jan", "Feb", "Mar", "Apr", "May", "Jun"]
        revenue_map = {int(row.month): float(row.total) for row in monthly_invoice_rows if row.month}
        revenue_labels = month_names
        revenue_values = [revenue_map.get(i, 0.0) for i in range(1, 7)]
    ...
    def analytics():
        ...
        invoices = (
            db.session.query(func.coalesce(func.sum(Invoice.amount), 0.0)).scalar()
            or 0.0
        )
    ...
    def create_client():
        ...
        try:
            ...
            db.session.rollback()
            flash("Unable to create client.", "error")
    ...
    # HRIS
    # =====================================================
    @app.route("/hris", methods=["GET", "POST"])
    def hris():
        if request.method == "POST":
            ...
            base_salary = parse_float(request.form.get("base_salary"), default=0.0)
            location = (request.form.get("location") or "").strip()
            ...
            employee = EmployeeProfile(
                ...
                base_salary=base_salary if base_salary else None,
                location=location or None,
                notes=notes or None,
            )
            ...
            try:
                ...
                db.session.rollback()
                flash("Unable to create employee profile.", "error")

            return redirect(url_for("hris"))

        employees = EmployeeProfile.query.order_by(EmployeeProfile.id.desc()).all()
        discipline = DisciplinaryRecord.query.order_by(DisciplinaryRecord.id.desc()).all()
        sop_requirements = SOPRequirement.query.order_by(SOPRequirement.id.desc()).all()
        open_hr_tasks = 0
    ...
    def create_point_log():
        ...
        point_log = PointLog(employee_id=employee_id, points=points, reason=reason)
        ...
        try:
            ...
            db.session.rollback()
            flash("Unable to create point log.", "error")

        return redirect(url_for("hris"))

    @app.route("/hris/discipline/create", methods=["POST"])
    def create_discipline():
        employee_id_raw = (request.form.get("employee_id") or "").strip()
        case_type = (request.form.get("case_type") or "").strip()
        severity = (request.form.get("severity") or "").strip()
        case_date = parse_date(request.form.get("case_date"))
        description = (request.form.get("description") or "").strip()
        action_taken = (request.form.get("action_taken") or "").strip()

        if not employee_id_raw or not case_type or not description:
            flash("Employee, case type, and description are required.", "error")
            return redirect(url_for("hris"))
        ...
        try:
            ...
            db.session.rollback()
            flash("Unable to create disciplinary record.", "error")

        return redirect(url_for("hris"))

    @app.route("/hris/sop/create", methods=["POST"])
    def create_sop():
        employee_id_raw = (request.form.get("employee_id") or "").strip()
        sop_title = (request.form.get("sop_title") or "").strip()
        linked_job_description = (request.form.get("linked_job_description") or "").strip()
        status = (request.form.get("status") or "").strip()
        due_date = parse_date(request.form.get("due_date"))
        notes = (request.form.get("notes") or "").strip()

        if not employee_id_raw or not sop_title:
            flash("Employee and SOP title are required.", "error")
            return redirect(url_for("hris"))
        ...
        sop = SOPRequirement(
            employee_id=employee_id,
            sop_title=sop_title,
            linked_job_description=linked_job_description or None,
        ...
        try:
            db.session.add(sop)
            db.session.commit()
            flash("SOP requirement created successfully.", "success")
        except SQLAlchemyError:
            db.session.rollback()
            flash("Unable to create SOP requirement.", "error")
    ...
    def create_lead():
        ...
        source = (request.form.get("source") or "").strip()
        stage = (request.form.get("stage") or "").strip()
        expected_value = parse_float(request.form.get("expected_value"), default=0.0)
        ...
        try:
            ...
            db.session.rollback()
            flash("Unable to create lead.", "error")
    ...
    def create_campaign():
        campaign_name = (request.form.get("campaign_name") or "").strip()
        channel = (request.form.get("channel") or "").strip()
        cost = parse_float(request.form.get("cost"), default=0.0)
        roi = (request.form.get("roi") or "").strip()
    ...
    def create_calendar_event():
        ...
        try:
            ...
            db.session.rollback()
            flash("Unable to create calendar event.", "error")
        ...
        return jsonify(
            {
                "assistant": "XIOMY",
                "status": "online",
                "message": "XIOMY executive intelligence layer active.",
            }
        )

    @app.route("/sami")
    def sami_redirect():
        return redirect(url_for("xiomy_page"))
